...
void IndTetraStream::init(){
	...
	if((mem_grid_size_ == -1) || (mem_grid_size_ > particle_grid_size_)){
	...
	total_tetra_grid_num_ =
        pow(ceil(particle_grid_size_ / (double)mem_grid_size_), 3);
...
IndTetraStream(std::string filename,
	...
	gsnap_ = new GSnap(filename, filename);
    
    if(gridsize == -1){
        int numparts = gsnap_->GetNpart(parttype);
        gridsize = ceil(pow(numparts, 1.0/3));
    }
    
	total_parts_ = gsnap_->GetNpart(parttype);
    
    mem_grid_size_ = memgridsize;
    particle_grid_size_ = gridsize;
    
    if(particle_grid_size_ % mem_grid_size_ != 0){
        printf("WARNING: mem_grid_size_ is not a divider of gridsize!\n");
    }
...
void IndTetraStream::readPosBlock(Point * &posblock,
