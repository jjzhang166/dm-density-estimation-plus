...
bool GridManager::loadGrid(int ind){
	if(ind < this->subgrid_num){
		if(ind == this->current_block_ind){
			...
			gridFile.read((char *) grid_, sizeof(double) * subgridsize_ * subgridsize_ * subgridsize_);
...
void GridManager::saveGrid(){
	...
	gridFile.write((char *) this->grid_, sizeof(double) * subgridsize_ * subgridsize_ * subgridsize_);
	gridFile.close();
}
