...
class Grids{
public:
	Grids(std::string filename, int gridsize, int subgridsize, GRIDTYPE init_v);
	Grids(std::string filename, int gridsize, int subgridsize, GRIDTYPE init_v,
	...
	GRIDTYPE getValueByActualCoor(int i, int j, int k); //get the value of the actual point. If current
											// block does not has the point, save and reload

	GRIDTYPE getValue(int i, int j, int k);	// get the value of current block

	void setValue(int i, int j, int k, GRIDTYPE value);// set the value of current block
	void setValueByActualCoor(int i, int j, int k, GRIDTYPE value); //set the value of the actual coor
	...
	GRIDTYPE * getSubGrid();					// get the actual subgrid memory
...
private:
	...
	void initialize(string filename, int gridsize, int subgridsize, GRIDTYPE init_v);
	...
	GRIDTYPE * grid_;							// the actual grid pointer
