...
//r1 < r2
void findPartsInBox(treeType &tree, Point & p,
                      Point vecP, REAL r,
                      vector<kdtreeNode> & retVec){
    kdtreeNode tempnod;
    tempnod.index = 0;
    ...
    Point finalP = p + vecP;
    //up
    region->_M_low_bounds[0] = finalP.x - r;
    region->_M_high_bounds[0] = finalP.x + r;
    
    region->_M_low_bounds[1] = finalP.y - r;
    region->_M_high_bounds[1] = finalP.y + r;
    
    region->_M_low_bounds[2] = finalP.z - r;
    region->_M_high_bounds[2] = finalP.z + r;
...
void printUsage(string pname){
    fprintf(stderr, "Usage: %s <Single Filename> <radius> <boxSize>\n",
            pname.c_str()
            );
    fprintf(stderr, "-OR-\n");
    fprintf(stderr, "%s <prefix> <basename> <numOfFiles> <radius> <boxSize>\n",
            pname.c_str()
            );
...
int main(int argv, char * args[]){
    ...
    fprintf(stderr, "boxSize = %f (kpc / h)\n", boxSize);
    ...
    for( size_t i = 0; i < nparts; i ++){
        ...
        vecP.z = radius;
        retVec.clear();
        findPartsInBox(kdtree, allpos[i],
                         vecP, boxSize/2,
                         retVec);
        ...
        for(size_t j = 0; j < retVec.size(); j ++){
            ...
            rvector = rvector / r;
            if(r >= radius - boxSize/2 && r < radius + boxSize/2){
